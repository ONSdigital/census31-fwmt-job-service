...
public interface GatewayCaseRecordRepository extends JpaRepository<GatewayCaseRecord, Long> {
  ...
  @Lock(LockModeType.PESSIMISTIC_WRITE)
  @Query("SELECT estab.caseId FROM GatewayCaseRecord estab WHERE estab.uprn = :estabUprn")
  String findByEstabUprn(@Param("estabUprn") String estabUprn);

  @Lock(LockModeType.PESSIMISTIC_WRITE)
  @Query("SELECT estab.caseId FROM GatewayCaseRecord estab WHERE estab.uprn = :uprn")
  String findByUprn(@Param("uprn") String uprn);

//  @Lock(LockModeType.PESSIMISTIC_WRITE)
//  @NonNull
//  List<GatewayCaseRecord> findAll();
